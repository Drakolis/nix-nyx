...
class ControlCenter(widgets.RevealerWindow):
  def __init__(self, monitor_id: int):
    ...
    username = widgets.Box(
      child=[
        widgets.Label(label=user_full_name, css_classes=["user-name"], halign="start"),
        widgets.Label(label=user_address, css_classes=["user-address"], halign="start"),
        widgets.Label(
          label=utils.Poll(timeout=60 * 1000, callback=lambda x: fetch.uptime).bind(
    ...
    revealer = widgets.Revealer(
      ...
      child=widgets.Box(
        css_classes=["flyout", "elevation3", "window"],
        spacing=12,
        width_request=500,
        vexpand=True,
        vertical=True,
        child=[
          ...
          widgets.Box(
            css_classes=["section", "user-profile"], child=[user_image, username]
          ),
          widgets.Box(
            css_classes=["section"],
            hexpand=True,
